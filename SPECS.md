 ...
 `*`|( n1 n2 --- n3 )|スタックトップ2ワードを乗算し結果を返す。演算は符号あり整数として行う。
 +|( n1 n2 --- n3 )|スタックトップ2ワードを加算し結果を返す。演算は符号あり整数として行う。
 +loop|( n --- )|`do ... +loop`構造。スタックの増分だけインデックスを増やす。 
 +rsp|( n --- )| rpsにnを足す。
 ,|( w --- )|スタックトップを辞書末尾(here)にコンパイルする
 -|( u1 u2 --- u )|u1 - u2をスタックトップに置く。
 -dup|( n --- n n (n != 0)|スタックトップが非0の場合dupする。
 -find|( c-addr -- c-addr 0 or<br> xt c 1 or<br> xt c -1 )|辞書検索。lastから検索する。見つからなければ、次のnumberに向けてaddrを残す。immediateであれば1、通常ワードであれば-1を置く。
 ."|( --- )|`." ..."`、文字列出力
 .(|( --- )|`.( ...)`、コンパイル時にテキスト出力
 ...
 boolean|( n -- ffff| or 0000 )|nをtrue/false値に変換 
 ...
 negate|( n -- 0000 or ffff )|論理否定値を返す。非0なら0を返す。0なら-1(0xffff)を返す。
 not|( n -- 0000 or ffff )|プリミティブだが挙動は`negate`と同じ。ちょっと変かも。論理否定値を返す。非0なら0を返す。0なら-1(0xffff)を返す。
