...
かなりいいところまで来た完全クロスコンパイル手法。今後、プライマリ辞書すげ替えによるセルフターゲット開発に移行する。user.defのヘッダ定義も相当数もとに戻すことになる。

せっかくここまで来たのだから記録として残しておきたい。現時点の`develop`ブランチを`fullcross`ブランチとして残しておき、これから`develop`をセルフクロスに戻す。

## ブランチ`RC`

C104原稿を脱稿した。C104頒布までに、本リポジトリのmainブランチを整えてゆく。当面クロスコンパイル残滓を残したまま、例題(ASCIIARTは入れたい)と文書整備を進める。

ブランチ`RC`はこのために用意した。今週末目途で`RC`を完成させ、日曜夜にアナウンスしてコミケに向け機運を高める。

## asciiart.f(8/2)

マンデルブロ図形が書けるようになった。高岡さんの`https://github.com/tgtakaoka/retro-bionic/tree/main`にある`mandelblot.c`の16bit integer C 言語版をほぼそのまま Forth に書き直したものである。

これを動作させることを通じて、加減乗除、`m*/` 演算が符号なしだったものを符号有りに変更する必要があった。従来 `m*/` を印字ルーチンで使っていたので、符号なし演算`um*/`を新たに作成した。

やはり、符号なし・符号ありの違いは明示的にコードに表さないといけない。
