...
* `start_cross.f`: ターゲット辞書のコンパイル準備、`CROSS_ADDR`にホスト辞書の`last`を保持し、`DP_ADDR`, `LAST_ADDR`は先ほどロードした`t_dict.f`ターゲット辞書のdp, lastを指すように更新する。合わせて、辞書イメージ先頭のユーザ変数初期値で、アセンブラで初期化できなかった部分も更新しておく。`S0_ADDR`, `R0_ADDR`, `TIB_ADDR`と、ユーザ変数領域と隣接する領域のポインタの初期化。
...
ユーザ変数をターゲット向け調整後に、ホスト側ユーザ変数にアクセスする箇所を確認する。
